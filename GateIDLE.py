    ...
    def operate(self):
        while True:
            ...
            for file in self.file_list:
                if file[3] == 1:
                    print(
                        "%-4d%-24s%-s" % (file[0], file[1], self.convert_size(file[2]))
                    )
                    count += 1
                    total_size += file[2]
            ...
            index = input(('Input index number to load or "q" to exit:'))
            if index == "q":
                os.system("cls")
                return True
            else:
...
class GateIDLE:
    def __init__(self):
        ...
        self.vf_klines = VirtualFolder(
            os.path.join(ds_dir, "klines"), self.db["klines"]
        )
